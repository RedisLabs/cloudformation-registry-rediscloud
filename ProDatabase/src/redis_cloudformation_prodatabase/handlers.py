...
def create_handler(
    ...
    # Creating the event dictionary that will be identical with a Swagger API call
        ...
        if (
            model.LocalThroughputMeasurement != ""
            and model.LocalThroughputMeasurement != None
        ):
            event["localThroughputMeasurement"] = json.loads(
                model.LocalThroughputMeasurement
            )
...
def delete_handler(
    ...
    try:
        ...
        if "404" in str(response_check):
            return ProgressEvent.failed(
                HandlerErrorCode.NotFound, f"Database with ID {db_id} was not found."
            )
        else:
            return ProgressEvent(
                status=OperationStatus.IN_PROGRESS,
...
def read_handler(
    ...
    if "databaseId" not in str(response) or response["databaseId"] != int(db_id):
        ...
        return ProgressEvent.failed(
            HandlerErrorCode.NotFound, f"Database {db_id} does not exist."
        )
    elif response["status"] == "deleting":
        return ProgressEvent.failed(
            HandlerErrorCode.NotFound, f"Database {db_id} is in delete state"
        )
    else:
        ...
        if "delete-draft" not in str(db):
            models.append(
                ResourceModel(
                    ...
                    UseExternalEndpointForOSSClusterApi=db.get(
                    ...
                    ClientTlsCertificates=db.get("security", {}).get(
                        "tlsClientAuthentication", None
                    ),
                    ...
                    EnableDefaultUser=db.get("security", {}).get(
                        "enableDefaultUser", None
                    ),
                    OnDemandBackup="",
                    RegionName="",
                    OnDemandImport="",
