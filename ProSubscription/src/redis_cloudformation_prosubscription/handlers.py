...
def DeleteSubscription (base_url, subscription_id, http_headers):
    ...
    time.sleep(10) #hardcoded 10 seconds of sleep because the delete call takes longer to be processed.
...
def read_handler(
    ...
    if 'id' not in str(response) or response['id'] != int(sub_id):
        ...
        return ProgressEvent.failed(
            HandlerErrorCode.NotFound,
            f"Subscription {sub_id} does not exist."
        )
    elif response["status"] == "deleting":
        return ProgressEvent.failed(
            HandlerErrorCode.NotFound,
            f"Subscription {sub_id} is in delete state"
        )
    else:
